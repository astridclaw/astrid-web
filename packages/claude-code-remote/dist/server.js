...
const astrid_client_1 = require("./astrid-client");
const repo_manager_1 = require("./repo-manager");
const index_1 = require("./executors/index");
...
app.get('/health', async (_req, res) => {
    const claudeAvailable = await claude_executor_1.claudeExecutor.checkAvailable();
    const openaiAvailable = !!process.env.OPENAI_API_KEY;
    const geminiAvailable = !!process.env.GEMINI_API_KEY;
    const activeSessions = await session_manager_1.sessionManager.getActiveSessions();
...
app.get('/test-executor', async (req, res) => {
    const testCwd = req.query.cwd || '/app/persistent/repos/Graceful-Tools/astrid-res-www';
    const taskTitle = req.query.title || 'Test task';
    const taskDescription = req.query.description || 'This is a test task description.';
    const maxTurns = parseInt(req.query.maxTurns || '1', 10);
    ...
    try {
        ...
        if (useFullFlow) {
            ...
            session = await session_manager_1.sessionManager.createSession({
                taskId: testTaskId,
                title: taskTitle,
            ...
            await session_manager_1.sessionManager.updateSession(testTaskId, { status: 'running' });
        ...
        const originalMaxTurns = claude_executor_1.claudeExecutor.maxTurns;
        claude_executor_1.claudeExecutor.maxTurns = maxTurns;
        // Build context similar to real webhook
        const testContext = {
            comments: req.query.comments ? JSON.parse(req.query.comments) : undefined,
            repository: req.query.repository || undefined,
            mcpToken: req.query.mcpToken || undefined,
            model: req.query.model || undefined
        };
        console.log(`🔧 Test context:`, JSON.stringify(testContext));
        const result = await claude_executor_1.claudeExecutor.startSession(session, undefined, testContext);
        claude_executor_1.claudeExecutor.maxTurns = originalMaxTurns;
        // Clean up test session if created via sessionManager
        if (useFullFlow) {
            await session_manager_1.sessionManager.deleteSession(session.taskId);
        }
        res.json({
            success: true,
...
app.get('/test-claude', async (req, res) => {
    ...
    const testCwd = req.query.cwd || undefined;
    const testPrompt = req.query.prompt || 'Respond with just the word OK';
    ...
    const testModel = req.query.model || 'opus';
    const testMaxTurns = req.query.maxTurns || '1';
    const args = [
        '--print',
        '--model', testModel,
...
app.post('/webhook', async (req, res) => {
    ...
    const verification = (0, webhook_signature_1.verifyWebhookSignature)(rawBody, signature, secret, timestamp);
...
app.get('/sessions', async (_req, res) => {
    const sessions = await session_manager_1.sessionManager.getAllSessions();
    res.json({
...
app.delete('/sessions/:taskId', async (req, res) => {
    const { taskId } = req.params;
    const session = await session_manager_1.sessionManager.getByTaskId(taskId);
    if (!session) {
        return res.status(404).json({ error: 'Session not found' });
    }
    console.log(`🗑️ Manually deleting session for task ${taskId} (was ${session.status})`);
    await session_manager_1.sessionManager.deleteSession(taskId);
    res.json({
        success: true,
...
app.post('/sessions/reset-stuck', async (_req, res) => {
    const sessions = await session_manager_1.sessionManager.getAllSessions();
    ...
    for (const session of sessions) {
        if (session.status === 'running') {
            ...
            if (updatedAt < oneHourAgo) {
                ...
                await session_manager_1.sessionManager.updateSession(session.taskId, { status: 'interrupted' });
...
async function handleTaskAssigned(payload) {
    ...
    const provider = (0, index_1.detectProvider)(aiAgent || {});
    const providerName = (0, index_1.getProviderName)(provider);
    ...
    let session = await session_manager_1.sessionManager.getByTaskId(task.id);
    if (session) {
        ...
        if (session.status === 'interrupted' || session.status === 'error') {
            ...
            await session_manager_1.sessionManager.deleteSession(task.id);
            session = undefined;
        }
    ...
    if (list?.githubRepositoryId) {
        ...
        const repoInfo = await repo_manager_1.repoManager.getRepo(list.githubRepositoryId);
        if (repoInfo) {
            projectPath = repoInfo.path;
            console.log(`✅ Repository ready at: ${projectPath}`);
            // Create task-specific branch
            try {
                const branchName = await repo_manager_1.repoManager.createTaskBranch(repoInfo.path, task.id);
    ...
    await session_manager_1.sessionManager.updateSession(task.id, { status: 'running' });
    // Notify Astrid that we've started
    await astrid_client_1.astridClient.notifyStarted(task.id, session.id, `Starting work on: ${task.title} (using ${providerName})`);
    // Get the appropriate executor for this provider
    const executor = (0, index_1.getExecutor)(provider);
    ...
    if (!isAvailable) {
        ...
        await session_manager_1.sessionManager.updateSession(task.id, { status: 'error' });
        await astrid_client_1.astridClient.notifyError(task.id, session.id, errorMsg);
        activeExecutions.delete(task.id);
    ...
    try {
        ...
        const onProgress = (message) => {
            astrid_client_1.astridClient.notifyProgress(task.id, session.id, message);
        };
        ...
        if (result.sessionId) {
            await session_manager_1.sessionManager.setClaudeSessionId(task.id, result.sessionId);
        }
        ...
        if (result.stdout.toLowerCase().includes('?') && !prUrl) {
            ...
            await session_manager_1.sessionManager.updateSession(task.id, { status: 'waiting_input' });
            await astrid_client_1.astridClient.notifyWaitingInput(task.id, session.id, parsed.summary || 'Need your input to continue', undefined, {
                files: allFiles,
            ...
            await session_manager_1.sessionManager.updateSession(task.id, { status: 'completed' });
            await astrid_client_1.astridClient.notifyCompleted(task.id, session.id, {
            ...
            // Error occurred
            await session_manager_1.sessionManager.updateSession(task.id, { status: 'error' });
            await astrid_client_1.astridClient.notifyError(task.id, session.id, parsed.error || `${providerName} exited with code ${result.exitCode}`, result.stderr);
        }
    }
    catch (error) {
        ...
        await session_manager_1.sessionManager.updateSession(task.id, { status: 'error' });
        await astrid_client_1.astridClient.notifyError(task.id, session.id, errorMessage);
    }
    finally {
...
async function handleCommentCreated(payload) {
    ...
    const session = await session_manager_1.sessionManager.getByTaskId(task.id);
    if (!session) {
    ...
    const provider = session.provider || (0, index_1.detectProvider)(aiAgent || {});
    const providerName = (0, index_1.getProviderName)(provider);
    const executor = (0, index_1.getExecutor)(provider);
    ...
    await session_manager_1.sessionManager.incrementMessageCount(task.id);
    await session_manager_1.sessionManager.updateSession(task.id, { status: 'running' });
    try {
        ...
        if (result.stdout.toLowerCase().includes('?') && !parsed.prUrl) {
            await session_manager_1.sessionManager.updateSession(task.id, { status: 'waiting_input' });
            await astrid_client_1.astridClient.notifyWaitingInput(task.id, session.id, parsed.summary || 'Need more input');
        }
        else if (result.exitCode === 0) {
            await session_manager_1.sessionManager.updateSession(task.id, { status: 'completed' });
            await astrid_client_1.astridClient.notifyCompleted(task.id, session.id, {
            ...
            await session_manager_1.sessionManager.updateSession(task.id, { status: 'error' });
            await astrid_client_1.astridClient.notifyError(task.id, session.id, parsed.error || `Exit code ${result.exitCode}`);
        }
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error';
 ...
 */
async function resumeInterruptedSession(session, message) {
    console.log(`🔄 Resuming interrupted session ${session.id}`);
    await session_manager_1.sessionManager.updateSession(session.taskId, { status: 'running' });
    try {
        const result = await claude_executor_1.claudeExecutor.resumeSession(session, message);
        const parsed = claude_executor_1.claudeExecutor.parseOutput(result.stdout);
        if (result.exitCode === 0) {
            await session_manager_1.sessionManager.updateSession(session.taskId, { status: 'completed' });
            await astrid_client_1.astridClient.notifyCompleted(session.taskId, session.id, {
                summary: parsed.summary,
                files: parsed.files,
                prUrl: parsed.prUrl
            });
        ...
        await session_manager_1.sessionManager.updateSession(session.taskId, { status: 'error' });
    }
}
/**
...
async function main() {
    ...
    await session_manager_1.sessionManager.recoverSessions();
    // Clean up old sessions
    await session_manager_1.sessionManager.cleanupExpired();
    // Start listening
