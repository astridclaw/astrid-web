...
export class OpenAIExecutor {
   ...
   */
  private async buildSystemPrompt(session: Session, context?: TaskContext): Promise<string> {
    let projectContext = ''
...
- If you need clarification, explain what you need to know`
  ...
  async startSession(
    ...
    let lastOutput = ''
    let completionMessage: { commit_message: string; pr_title: string; pr_description: string } | undefined
        ...
        if (assistantMessage.tool_calls && assistantMessage.tool_calls.length > 0) {
          for (const toolCall of assistantMessage.tool_calls) {
            ...
            if (toolName === 'task_complete') {
              completionMessage = args as typeof completionMessage
              ...
              if (session.projectPath && modifiedFiles.length > 0) {
                try {
                  ...
                  execSync(`git commit -m "${args.commit_message}"`, { cwd: session.projectPath })

                  // Try to create PR
                  try {
                    const prOutput = execSync(
                      `gh pr create --title "${args.pr_title}" --body "${args.pr_description.replace(/"/g, '\\"')}"`,
